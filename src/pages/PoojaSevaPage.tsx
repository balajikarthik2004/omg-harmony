  ...
  return (
    <div className="pooja-premium space-y-6 max-w-[1500px] mx-auto animate-fade-in relative">
      <div className="page-header-banner poojs-header">
        <div>
          <h1 className="text-2xl font-display font-bold text-foreground flex items-center gap-2">Pooja & Seva Management</h1>
          ...
          <section className="section-panel pooja-main-panel shadow-sm">
            ...
            <div className="px-4 py-3 bg-muted/10 border-b border-border/60 flex flex-wrap gap-2">
              {categorySummary.map(item => (
                ...
                >
                  {item.category} <span className={`min-w-5 px-1.5 py-0.5 rounded-full text-[10px] text-center font-extrabold ${activeCategory === item.category ? 'bg-primary-foreground/20 text-primary-foreground border border-primary-foreground/30 dark:bg-white/25 dark:text-white dark:border-white/30' : 'bg-primary border-primary text-primary-foreground shadow-md scale-[1.02]'}`}>{item.count}</span>
                </button>
              ))}
            </div>
      ...
      <Modal open={!!receiptItem} onClose={() => setReceiptItem(null)} title="Temple Booking Receipt">
        {receiptItem && (
          <div className="pooja-receipt-context space-y-5 px-1 pb-1">
            <div className="pooja-receipt-shell rounded-2xl border border-border p-6 bg-gradient-to-br from-card to-muted/30 relative overflow-hidden shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-y-5 gap-x-6 text-sm relative z-10">
                ...
                <div><p className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground mb-1">Date of Seva</p><p className="font-bold text-foreground">{formatDateDDMMYYYY(receiptItem.date)}</p></div>
                <div className="col-span-2 bg-muted/40 p-4 rounded-xl border border-border shadow-sm group hover:border-primary/30 transition-colors">
                  <p className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground mb-1">Pooja Configuration</p>
                  <p className="font-bold text-lg text-foreground mb-1">{receiptItem.poojaType}</p>
                  <p className="text-xs text-muted-foreground font-semibold flex items-center gap-1.5"><Ticket className="w-3.5 h-3.5 text-muted-foreground" /> For Devotee: <span className="text-foreground">{receiptItem.devoteeName}</span></p>
                </div>
              ...
              <div className="mt-6 pt-5 border-t-2 border-dashed border-border/80 flex justify-between items-center relative z-10">
                ...
                <p className="text-2xl font-black font-display text-emerald-500 bg-emerald-500/10 px-4 py-1.5 rounded-lg border border-emerald-500/20">{money(getPoojaDetails(receiptItem.poojaType, poojaCatalog)?.amount || 0)}</p>
              </div>
            </div>
