      ...
      <div className={`relative border-b border-white/10 backdrop-blur-sm bg-white/[0.03] ${isCollapsed ? 'p-3' : 'p-5'}`}>
        <div className={`flex ${isCollapsed ? 'flex-col items-center gap-2' : 'items-start justify-between gap-3'}`}>
          <button
            ...
            className={`h-7 w-7 rounded-md border border-white/20 bg-white/10 opacity-80 hover:opacity-100 hover:bg-white/20 transition-all duration-200 hover:shadow-[0_8px_20px_rgba(0,0,0,0.22)] flex items-center justify-center ${
        ...
        {!isCollapsed && (
          <p className="mt-2 text-[11px] font-semibold tracking-[0.12em] uppercase opacity-70" style={{ color: 'hsl(var(--sidebar-foreground))' }}>Temple Governance System</p>
        )}
      </div>

      {/* Navigation */}
      <nav className={`relative flex-1 py-4 overflow-y-auto ${isCollapsed ? 'px-2' : 'px-3.5'}`}>
        {!isCollapsed && <p className="px-2 pb-2 text-[10px] font-semibold tracking-[0.18em] uppercase opacity-45" style={{ color: 'hsl(var(--sidebar-foreground))' }}>Workspace</p>}
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `group relative isolate flex items-center rounded-xl border text-sm font-medium transition-all duration-200 ${
                  ...
                  isActive
                    ? 'border-white/25 bg-[linear-gradient(140deg,rgba(255,255,255,0.22),rgba(255,255,255,0.1))] shadow-[inset_0_1px_0_rgba(255,255,255,0.35),0_10px_18px_rgba(0,0,0,0.22)] -translate-y-[1px]'
                    : 'border-transparent opacity-75 hover:opacity-100 hover:bg-white/10 hover:border-white/20 hover:-translate-y-[1px] hover:shadow-[0_8px_18px_rgba(0,0,0,0.18)]'
                }`
              }
              style={{ color: 'hsl(var(--sidebar-foreground))' }}
            >
        ...
        <div className={`flex items-center rounded-xl bg-white/10 border border-white/15 ${isCollapsed ? 'justify-center px-2 py-2.5' : 'gap-3 px-3 py-2.5'}`}>
          ...
          {!isCollapsed && (
            <div className="min-w-0 flex-1">
              <p className="text-xs font-semibold opacity-90 truncate" style={{ color: 'hsl(var(--sidebar-foreground))' }}>{user?.name}</p>
              <p className="text-[10px] tracking-wide opacity-55 capitalize" style={{ color: 'hsl(var(--sidebar-foreground))' }}>{user?.role}</p>
            </div>
          )}
        </div>
        <button
          ...
          className={`group flex items-center rounded-xl text-sm font-medium opacity-75 hover:opacity-100 hover:bg-white/10 border border-transparent hover:border-white/20 w-full transition-all duration-200 hover:-translate-y-[1px] ${
            isCollapsed ? 'justify-center px-2 py-2.5' : 'gap-3 px-3 py-2.5'
          }`}
          style={{ color: 'hsl(var(--sidebar-foreground))' }}
        >
